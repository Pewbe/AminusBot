...
public class DBManager {
    ...
    private final String DB_URL = "jdbc:mysql://localhost/test?serverTimezone=UTC"; //접속할 DB 서버 ?serverTimezone=UTC는 mysql8이상일때 넣어줘야함
    ...
    public DBManager(){
        ...
        try{
            ...
            e.printStackTrace();
        } finally { //예외가 있든 없든 무조건 실행. state와 conn이 만약 닫히지 않았을 때 제대로 닫아주는건듯
            try{
                if(state!=null)
                    state.close();
